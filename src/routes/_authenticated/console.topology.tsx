        ...
        <div className="grid gap-4 lg:grid-cols-[1fr_320px]">
          <div className="surface-panel relative overflow-hidden">
            <svg viewBox={`0 0 ${W} ${H}`} className="w-full">
              ...
              {state.links.map((l) => {
                const a = positions[l.from],
                  b = positions[l.to];
                if (!a || !b) return null;
                const sevA = scores[l.from]?.severity,
                  sevB = scores[l.to]?.severity;
                const stroke =
                  sevA === "critical" || sevB === "critical"
                    ? "var(--color-status-critical)"
                    : sevA === "warning" || sevA === "high" || sevB === "warning" || sevB === "high"
                ...
                return (
                  <line
                    ...
                    y2={py(b.y)}
                    stroke={stroke}
                    strokeWidth={l.primary ? 1.4 : 0.8}
                    strokeDasharray={l.primary ? "" : "4 4"}
                    opacity={0.7}
                  />
                );
              })}
                ...
                return (
                  <g key={d.id} className="cursor-pointer" onClick={() => setSelected(d.id)}>
                    <circle
                      ...
                      r={r + 5}
                      fill={color}
                      opacity={s.severity === "critical" ? 0.18 : 0}
                    />
                    <circle
                      cx={px(p.x)}
                      cy={py(p.y)}
                      r={r}
                      fill={color}
                      stroke={isSel ? "var(--color-foreground)" : "var(--color-background)"}
            ...
            <div className="pointer-events-none absolute inset-x-0 bottom-0 flex flex-wrap gap-3 border-t border-border bg-background/80 px-3 py-2 text-[10px] font-mono uppercase tracking-wider text-muted-foreground backdrop-blur">
              {(["healthy", "observe", "warning", "high", "critical", "offline"] as const).map(
                (s) => (
                  <span key={s} className="inline-flex items-center gap-1.5">
                    <span
                      className="status-dot"
                      style={{ backgroundColor: severityColor(s), color: severityColor(s) }}
                    />
                    {severityLabel(s)}
                  </span>
                ),
          ...
          <Section title={selectedDevice ? "Selected device" : "Inspector"}>
            {!selectedDevice && (
              <p className="text-sm text-muted-foreground">
                Click a node to inspect its current state and recommended actions.
              </p>
            )}
            {selectedDevice && selectedScore && (
              <div className="space-y-3">
                ...
                <div className="text-xs text-muted-foreground">
                  ...
                  {selectedDevice.siteId.toUpperCase()}
                </div>
