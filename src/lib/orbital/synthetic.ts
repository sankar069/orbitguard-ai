...
import type {
  Device, Link, Site, TelemetryPoint, ActiveFault, FaultType, DeviceType,
} from "./types";
...
export const sites: Site[] = [
  { id: "mcc",      name: "Mission Control Centre",     shortCode: "MCC", region: "Bengaluru",  coord: { x: 0.5,  y: 0.5  } },
  { id: "gs-north", name: "Ground Station North",       shortCode: "GSN", region: "Lucknow",    coord: { x: 0.4,  y: 0.18 } },
  { id: "gs-south", name: "Ground Station South",       shortCode: "GSS", region: "Trivandrum", coord: { x: 0.42, y: 0.84 } },
  { id: "dpc",      name: "Data Processing Centre",     shortCode: "DPC", region: "Hyderabad",  coord: { x: 0.78, y: 0.42 } },
  { id: "boc",      name: "Backup Operations Centre",   shortCode: "BOC", region: "Pune",       coord: { x: 0.18, y: 0.62 } },
  { id: "soc",      name: "Security Operations Centre", shortCode: "SOC", region: "Bengaluru",  coord: { x: 0.7,  y: 0.7  } },
];
...
interface DeviceSpec { type: DeviceType; count: number; }
const layout: Record<string, DeviceSpec[]> = {
  mcc:      [{ type: "core-router", count: 1 }, { type: "switch", count: 1 }, { type: "server", count: 1 }],
  "gs-north": [{ type: "edge-router", count: 1 }, { type: "optical-mux", count: 1 }, { type: "switch", count: 1 }],
  "gs-south": [{ type: "edge-router", count: 1 }, { type: "optical-mux", count: 1 }, { type: "switch", count: 1 }],
  dpc:      [{ type: "core-router", count: 1 }, { type: "server", count: 2 }],
  boc:      [{ type: "edge-router", count: 1 }, { type: "switch", count: 1 }],
  soc:      [{ type: "firewall", count: 1 }, { type: "server", count: 1 }],
};
  ...
  for (const site of sites) {
    ...
    for (const spec of specs) {
      for (let i = 0; i < spec.count; i++) {
        ...
        const lastMaintenance = new Date(Date.now() - Math.floor(rng() * 90) * 86400000).toISOString();
        devices.push({
          id, name: id, type: spec.type, siteId: site.id,
          vendor, model,
          serial: `SN${Math.floor(rng() * 1e8).toString(36).toUpperCase()}`,
          firmware: `${4 + Math.floor(rng() * 4)}.${Math.floor(rng() * 12)}.${Math.floor(rng() * 30)}`,
          installedOn, lastMaintenance,
        });
      }
    }
  }
...
export function buildLinks(devices: Device[]): Link[] {
  ...
  links.push({ id: `L${pad(++lid, 3)}`, from: gsn.id, to: gss.id, bandwidthGbps: 40, primary: false });
  links.push({ id: `L${pad(++lid, 3)}`, from: boc.id, to: dpc.id, bandwidthGbps: 40, primary: false });
  ...
  for (const site of sites) {
    ...
    for (const d of siteDevices) {
      ...
      links.push({ id: `L${pad(++lid, 3)}`, from: sw.id, to: d.id, bandwidthGbps: 10, primary: true });
    }
  }
  return links;
}
...
interface DeviceBaseline {
  latency: number; jitter: number; loss: number; util: number;
  cpu: number; mem: number; temp: number; opticalRx: number;
  errBase: number;
}
...
export function appendTick(state: SyntheticState, simSpeed = 1): SyntheticState {
  const now = Date.now();
  const next: SyntheticState = { ...state, history: { ...state.history }, faults: [...state.faults] };
  ...
  for (const d of state.devices) {
    ...
    const newBuf = buf.length >= HISTORY_POINTS
      ? [...buf.slice(1), p]
      : [...buf, p];
...
export function injectFault(state: SyntheticState, deviceId: string, type: FaultType): SyntheticState {
